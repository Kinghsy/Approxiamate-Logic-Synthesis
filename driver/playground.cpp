...
int main() {
    ...
    auto net0 = BlifBooleanNet(McncAigPath / fBlif("apex2"));

    auto net1 = BlifBooleanNet(TempPath / fBlif("apex2__3"));
    auto net2 = BlifBooleanNet(TempPath / fBlif("apex2__11"));
...
//    }
    ...
    auto& abc = ExAbc::getInstance();
    abc.loadBlif(McncAigPath / fBlif("apex2"));
    abc.resyn2();
    abc.map();
    std::cout << abc.postMapArea() << std::endl;

    abc.reboot();
    abc.loadBlif(TempPath / fBlif("apex2__11"));
    abc.resyn2();
    abc.map();
    std::cout << abc.postMapArea() << std::endl;
